...
func TestBackup2B(t *testing.T) {
	...
	Backup2BTestPrintf("增加一条command")
	cfg.one(rand.Int(), servers, true)

	// put leader and one follower in a partition
	leader1 := cfg.checkOneLeader()
	Backup2BTestPrintf("断开3个节点，只留下Leader=%d,和一个Follower=%d", leader1, (leader1+1)%servers)

	cfg.disconnect((leader1 + 2) % servers)
	cfg.disconnect((leader1 + 3) % servers)
	cfg.disconnect((leader1 + 4) % servers)
	Backup2BTestPrintf("增加50条command, 不应该commit")
	...
	Backup2BTestPrintf("Leader=%d,Follower=%d 断开连接", leader1, (leader1+1)%servers)

	cfg.disconnect((leader1 + 0) % servers)
	cfg.disconnect((leader1 + 1) % servers)
	Backup2BTestPrintf("Server=%d, Server=%d,Server=%d 加入连接", (leader1+2)%servers, (leader1+3)%servers, (leader1+4)%servers)
	// allow other partition to recover
	cfg.connect((leader1 + 2) % servers)
	cfg.connect((leader1 + 3) % servers)
	cfg.connect((leader1 + 4) % servers)

	Backup2BTestPrintf("增加50条commnad,应该commit")
	...
	Backup2BTestPrintf("Follower=%d 断开连接", other)
	Backup2BTestPrintf("增加50条command, 不应该commit")
	...
	Backup2BTestPrintf("全部节点断开连接")

	for i := 0; i < servers; i++ {
		cfg.disconnect(i)
	}
	Backup2BTestPrintf("Server=%d,Server=%d,Server=%d 重新连接", (leader1+0)%servers, (leader1+1)%servers, other)
	cfg.connect((leader1 + 0) % servers)
	cfg.connect((leader1 + 1) % servers)
	cfg.connect(other)
	leader3 := cfg.checkOneLeader()
	Backup2BTestPrintf("Server=%d 成为Leader，期望的Leader是%d", leader3, other)
	// lots of successful commands to new group.
	Backup2BTestPrintf("增加50条command, 应该commit")
	for i := 0; i < 50; i++ {
		cfg.one(rand.Int(), 3, true)
	}
	Backup2BTestPrintf("全部节点加入连接")
	// now everyone
	for i := 0; i < servers; i++ {
		cfg.connect(i)
	}
	Backup2BTestPrintf("增加1条command，应该commit")
	cfg.one(rand.Int(), servers, true)
